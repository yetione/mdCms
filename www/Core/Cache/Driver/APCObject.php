...
class APCObject extends MemoryObject implements IMemoryStorage
{
	...
	{
		$this->set_ID($ID);
        // TODO: Исправить этот жуткий костыль
        //new KeyAutoUnLocker(array($this, 'unlock_key'));
        //unset($a);
	}
	...
	public function lock_key($key, &$auto_unlocker_variable)
	{
        ...
        var_dump($auto_unlocker_variable);
		return true;
	}
    ...
    public function get_by_tags($tags)
    {
        ...
        foreach ($i as $key_tags)
        {
            ...
            {
                $intersect = array_intersect($tags, $key_tags[self::apc_arr_value]);
                if (!empty($intersect)) $toget[] = substr($key_tags[self::apc_arr_key], $l);
            }
        }
        return !empty($toget) ?  $this->read($toget) : array();
    }
}
