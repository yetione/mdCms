...
class RedisObject extends MemoryObject implements IMemoryStorage
{
    ...
    public function get_by_tags($tags)
    {
        ...
        {
            $keys = $this->redis->sMembers($this->tag_prefix.$tag);
            if (!empty($keys)) array_merge($toget, $keys);
            else $this->redis->Del($this->tag_prefix.$tag);
        }
        return !empty($toget) ? array_fill_keys($toget, $this->read($toget)) : array();
        //return $d;
    }
    ...
    {
        // TODO: Implement getKeyTTL() method.
    }
}
