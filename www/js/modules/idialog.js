//TODO:Два раза активируется контроллер диалога. Разобраться
/**
 * iDialog main module
 */
 ...
 */
    .run(['$templateCache', 'idialogWindowTpl', function($templateCache, idialogWindowTpl) {
        $templateCache
            .put(idialogWindowTpl, '<div class="idialog animated ng-hide" ng-show="visible">'
                ...
                + '<div class="w1" ng-init="startLoading()"></div>'
                + '</div>');
    }])
    ...
    .service('$idialog', ['$compile', '$timeout', '$rootScope', function($compile, $timeout, $rootScope) {
        return function(template, options) {
            ...
            $timeout(function() {
                ...
                newScope.dialogOptions = options.options || {};
    ...
    .directive('idialogWindow', ['$templateCache', '$timeout', '$compile', '$http', 'idialogWindowTpl', function($templateCache, $timeout, $compile, $http, idialogWindowTpl) {
        return {
            ...
            controller: ['$scope', '$element', '$rootScope', '$q', function($scope, $element, $rootScope, $q) {
                activate();
                function activate() {
                    if ($scope.dialogOptions.onDialogActivate){
                        $scope.dialogOptions.onDialogActivate($scope);
                    }
                }
                /**
                ...
                $scope.show = function() {
                    var f = function () {
                        ...
                        if ($scope.scopeEventsEnabled) {
                            $scope.$broadcast('show', $scope);
                            $rootScope.$broadcast('iDialogShow', $scope.dialogId);
                        }
                    }
                    if ($scope.dialogOptions.beforeShow){
                        var deferred = $q.defer();
                        deferred.promise.then(function(r){f();}, function (r) {$scope.$overlay.remove();$element.remove();});
                        $scope.dialogOptions.beforeShow($scope, deferred);
                    }else{
                        f();
                    }
                };
                ...
                $scope.hide = function() {
                    var f = function () {
                        ...
                        if ($scope.scopeEventsEnabled) {
                            $scope.$broadcast('hide', $scope);
                            $rootScope.$broadcast('iDialogHide', $scope.dialogId);
                        }
                    };

                    if ($scope.dialogOptions.beforeHide){
                        var deferred = $q.defer();
                        deferred.promise.then(function(r){
                            f();
                        }, function (r) {});
                        $scope.dialogOptions.beforeHide($scope, deferred);
                    }else{
                        f();
                    }
